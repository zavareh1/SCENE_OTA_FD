...
def evaluate_model(model, dataloader, device="cuda:0"):
    ...
    for data, targets in dataloader:
        ...
        targets = targets.to(device, non_blocking=True).long()  # <-- add .long()
...
def evaluate_model_classwise(
    ...
    for data, targets in dataloader:
        ...
        for class_idx in range(num_classes):
            ...
            classwise_count[class_idx] += class_elem.sum().item() # samples with label c
            classwise_correct[class_idx] += (targets == preds)[class_elem].sum().item()# of those predicted correctly

    classwise_accuracy = classwise_correct / classwise_count
    #the mean of per-class accuracies (macro-avg), rounded to 4 decimals
    accuracy = round(classwise_accuracy.mean().item(), 4)
    #this is not the same as micro accuracy unless class counts are uniform
...
def get_round_personalized_acc(round_results, server_results, data_distributed):
    ...
    result_dict["in_dist_acc_mean"] = (round_in_dist_acc).mean().item() # size-weighted mean of in-dist acc
    result_dict["in_dist_acc_std"] = (round_in_dist_acc).std().item()# plain mean
    result_dict["out_dist_acc"] = torch.dot(round_out_dist_acc, local_size_prop).item() # size-weighted mean of out-dist acc
