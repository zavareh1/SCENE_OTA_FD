...
class BaseClientTrainer:
    ...
    def train(self):
        ...
        for _ in range(self.local_epochs):
            for data, targets in self.trainloader:
                ...
                targets = targets.to(self.device, non_blocking=True).long()  # <-- add .long()
    ...
    def download_global(self, server_weights, server_optimizer):
        ...
        # sanity check: ensure lr > 0
        for g in self.optimizer.param_groups:
            assert g["lr"] > 0, "Loaded optimizer has lr=0; did the server optimizer state load properly?"
    ...
    def reset(self):
        ...
        self.testloader = None
        self.optimizer = torch.optim.SGD(self.model.parameters(), lr=0)
        if hasattr(self, "dg_model"):
            del self.dg_model
            self.dg_model = None
            torch.cuda.empty_cache()  # optional on CUDA
